...
### Run the integration simulation (Icarus Verilog)

```bash
cd tb
iverilog -g2012 -o sim ../rtl/*.sv tb_proxcore_top.sv
vvp sim
```

### Reproduce the GDSII (OpenLane 2)
...
The flow produces the final GDSII, LEF, and netlist matching the artifacts already committed in `gds/`.
...
| Synthesis & PnR | Yosys + OpenROAD via OpenLane 2 |
